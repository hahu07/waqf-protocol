...
export const Passkey = ({
  ...
  className,
  loading = false,
  loadingText = 'Authenticating with Passkey'
}: PasskeyProps) => {
  ...
  useEffect(() => {
    isWebAuthnAvailable().then(setPasskeySupported);
  }, []);
  ...
  const onProgress = (progress: PasskeyProgress | undefined) => {
    ...
    if (progress && 'success' in progress) onSuccess().finally(close);
    if (progress && 'error' in progress) onError(progress.error);
  };
